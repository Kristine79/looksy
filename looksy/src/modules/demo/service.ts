...
export async function loadDemoContent(userId: string): Promise<LoadDemoResult> {
  ...
  const existing = await closetRepository.findItems(userId, { status: "active", limit: 1 });
  if (existing.length > 0) {
    return { status: "skipped", itemCount: 0, outfitCount: 0, memoryCount: 0 };
  }
  ...
  const outfitsRepository = new OutfitsRepository(db);
  let outfitCount = 0;
  ...
  logger.info("demo_content_loaded", { userId, itemCount: itemIds.length, outfitCount, memoryCount });
